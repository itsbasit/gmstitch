...
@section('content')
    <div>
        <a href="{{ route('stockcustomer.index') }}" class="btn btn-primary rounded-pill mt-4 ml-3"> Back To List</a>
    </div>
    <div class="container">
        ...
        @foreach ($data as $value)
            ...
            <tr>
                <h3 class="text-center text-dark bg-white w-40">
                    ...
                    <td> Brand:{{ $value->brand }}</td> <br>
                    {{-- <td>{{ $value->size }}</td> <br> --}}
                    <td> {{ $value->brandtype }}</td> <br>
                    <td> Total Price: {{ $value->totalprice }}</td> <br>
